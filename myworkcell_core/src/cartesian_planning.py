...
def client_2():
	...
	display_trajectory_publisher = rospy.Publisher('/move_group/display_planned_path',
                                               moveit_msgs.msg.DisplayTrajectory,
                                               queue_size=20)
	#use a list to store all waypoints 
	waypoints = []

	#set a variable wpose equal to the move_target
	wpose = move_target
	
	#create a scale of one 
	scale = 1

	# First move down (z) and append the waypoint
	wpose.position.z -= scale * 0.1  
	waypoints.append(copy.deepcopy(wpose))

	# Second move forward (x) and append the waypoint
	wpose.position.x += scale * 0.1  
	waypoints.append(copy.deepcopy(wpose))

	# Third move sideways (y) and append
	...
	print("start Cartesian path planning")
